...
function mixinMigration(MsSQL) {
          ...
          fields.forEach(function (f) {
            ...
            if (f.Length) {
              f.Type = f.Type + '(' + f.Length + ')';
               ...
               f.Type = `${f.Type}(${f.Precision}, ${f.Scale || 0})`;
            }
          });
        }
        cb && cb(err, fields);
  ...
  MsSQL.prototype.showIndexes = function (model, cb) {
    ...
    const sql = 'SELECT OBJECT_SCHEMA_NAME(T.[object_id],DB_ID()) AS [table_schema],' +
      ...
      ' I.[allow_row_locks], I.[allow_page_locks], IC.[is_descending_key], IC.[is_included_column], IC.[index_column_id]' +
  ...
  MsSQL.prototype.getColumnsToAdd = function (model, actualFields) {
    ...
    const columnNames = {};
    propNames.forEach((propName) => { columnNames[propName] = (m.properties[propName].mssql && m.properties[propName].mssql.columnName) ? m.properties[propName].mssql.columnName : propName})
    ...
    propNames.forEach(function (propName) {
      ...
      if (actualFields) {
        actualFields.forEach(function (f) {
          if (f.Field === propName || f.Field.toUpperCase() === columnNames[propName].toUpperCase()) {
            found = f;
          }
        });
      }
    ...
    function changed(newSettings, oldSettings) {
      ...
      //TODO - rename columns
      // if (newSettings.mssql && newSettings.mssql.columnName && newSettings.mssql.columnName !== oldSettings.Field) {
      //   return true;
      // } 

      if (oldSettings.Null === 'YES' &&
        (newSettings.allowNull === false || newSettings.null === false || (newSettings.mssql && newSettings.mssql.nullable === 'NO'))) {
        return true;
      }
      if (oldSettings.Null === 'NO' && (newSettings.allowNull === true ||
        newSettings.null === true || (newSettings.mssql && newSettings.mssql.nullable === 'YES'))) {
        return true;
      }
      if (
        oldSettings.Type !== datatype(newSettings, true)
      ) {
        return true;
      }
      return false;
  ...
  MsSQL.prototype.getColumnsToDrop = function (model, actualFields) {
    ...
    const columnNamesToPropNames = {};
    propNames.forEach((propName) => { 
      let columnName = propName;
      if (m.properties[propName].mssql && m.properties[propName].mssql.columnName) {
        columnName = m.properties[propName].mssql.columnName;
      } 
      columnNamesToPropNames[columnName.toUpperCase()] = propName;
    })
    ...
    if (actualFields) {
      // drop columns
      actualFields.forEach(function (f) {
        const notFound = !~propNames.indexOf(f.Field) && !columnNamesToPropNames[f.Field.toUpperCase()];
        if (f.Field === idName) return;
        if (notFound || !m.properties[columnNamesToPropNames[f.Field.toUpperCase()]]) {
  ...
  MsSQL.prototype.addIndexes = function (model, actualIndexes) {
    ...
    if (actualIndexes) {
      actualIndexes.forEach(function (i) {
        ...
        ai[name].columns[i.index_column_id - 1] = (i.Column_name);
      });
    }
    ...
    aiNames.forEach(function (indexName) {
      ...
      //Is this a single column index? defined in property.index
      
      var isSingleColumnIndex = (indexNames.indexOf(indexName) === -1);
      
      if (isSingleColumnIndex ){
        var propName = indexName.split('_')[0];
        if (!m.properties[propName] || (m.properties[propName] && !m.properties[propName].index)) {
            dropIndex = true;
            SQL = 'DROP INDEX ' + indexName + ' ON ' + self.tableEscaped(model);
         } 
      }
      // second: check multiple indexes? defined in model.settings.indexes
      ...
      if (!isSingleColumnIndex) {
        ...
        m.settings.indexes[indexName].columns.split(/,\s*/).forEach(function (columnName, i) {
          if (ai[indexName].columns[i] !== columnName) {
            orderMatched = false;
            dropIndex = true;
            SQL = 'DROP INDEX ' + self.columnEscaped(model, indexName) + ' ON ' + self.tableEscaped(model);
          }
        });
      }

      if (!orderMatched || dropIndex ) {
        sql.push(SQL);
        delete ai[indexName];
      }
    ...
    propNames.forEach(function (propName) {
      ...
      if (!found) {
        ...
        const allowNulls = isNullable(m.properties[propName]);
        if (!i) {
          return;
        }
        let type = 'ASC';
        ...
        let cmd = 'CREATE ' + (unique ? 'UNIQUE ' : '') + kind + ' INDEX [' + name + '] ON ' +
          ...
          cmd += '(' + MsSQL.newline;
          cmd += '    ' + self.columnEscaped(model, propName) + ' ' + type;
          cmd += MsSQL.newline + ')'
          if (allowNulls) cmd += ' WHERE ' + self.columnEscaped(model, propName) + ' IS NOT NULL ';
          cmd+= 
    ...
    indexNames.forEach(function (indexName) {
      ...
      if (!found) {
        ...
        self._idxNames[model].push(indexName);

        let cmd = 'CREATE ' + (unique ? 'UNIQUE ' : '') + kind + ' INDEX [' + indexName + '] ON ' +
          tblName + MsSQL.newline;
        cmd += '(' + MsSQL.newline;
  ...
  MsSQL.prototype.alterTable = function (model, actualFields, actualIndexes, done, checkOnly) {
    ...
    if (statements.length) console.debug('MsSQL.prototype.alterTable()', model, statements);
    if (checkOnly) {
      done(null, !statements.length);
      return;
    }
  ...
  MsSQL.prototype.singleIndexSettingsSQL = function (model, prop, add) {
    ...
    let cmd = 'CREATE ' + (unique ? 'UNIQUE ' : '') + kind + ' INDEX [' + name + '] ON ' +
      tblName + MsSQL.newline;
    cmd += '(' + MsSQL.newline;
    cmd += '    ' + this.columnEscaped(model, prop) + ' ' + type;
    cmd += MsSQL.newline + ') WITH (PAD_INDEX  = OFF, STATISTICS_NORECOMPUTE = OFF,' +
      ' SORT_IN_TEMPDB = OFF, IGNORE_DUP_KEY = OFF, DROP_EXISTING = OFF, ' +
      'ONLINE = OFF, ALLOW_ROW_LOCKS  = ON, ALLOW_PAGE_LOCKS  = ON);' +
      MsSQL.newline;
  ...
  function isNullable(p) {
    return !(p.required || p.id || p.nullable === false ||
      p.allowNull === false || p['null'] === false || (p.mssql && p.mssql.nullable === 'NO'));
  }
  ...
  MsSQL.prototype.columnDataType = function (model, property, formatNumbers = false) {
    ...
    if (colType) {
      ...
      if (dataPrecision && dataScale && formatNumbers) {
    ...
    return datatype(prop, formatNumbers);
  };

  function datatype(p, formatNumbers = false) {
    let dt = '';
    var typeLabel = null
    // p = normalisePropDefinition(p);

    if (p.mssql) {
      if (p.mssql.typeDef) return `[${p.mssql.typeDef}]`
      if (typeof p.mssql.dataType === "string") typeLabel = p.mssql.dataType;
    }
      ...
      switch (p.type.name) {
        ...
        case `Number`:
          dt = "[int]";
          if (formatNumbers || p.precision) {
            dt = `[int](${p.precision || 10}, ${p.scale || 0})`;
          }
          break;
        case `Date`:
          dt = `[${typeLabel || 'datetime'}]`;
          break;
        case `Boolean`:
          dt = `[bit]`;
          break;
