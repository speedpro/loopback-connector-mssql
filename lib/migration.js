...
function mixinMigration(MsSQL) {
  ...
  MsSQL.prototype.getColumnsToAdd = function (model, actualFields) {
    ...
    propNames.forEach(function (propName) {
      ...
      if (actualFields) {
        actualFields.forEach(function (f) {
          if (f.Field === propName || f.Field === columnNames[propName]) {
            found = f;
          }
        });
      }
    ...
    function changed(newSettings, oldSettings) {
      ...
      if (oldSettings.Null === 'NO' && !(newSettings.allowNull === false ||
        newSettings.null === false || (newSettings.mssql && newSettings.mssql.nullable === 'NO'))) {
        return true;
      }
  ...
  MsSQL.prototype.getColumnsToDrop = function (model, actualFields) {
    ...
    propNames.forEach((propName) => { 
      if (m.properties[propName].mssql && m.properties[propName].mssql.columnName) {
        let columnName = m.properties[propName].mssql.columnName;
        columnNamesToPropNames[columnName] = propName;
      } else {
        columnNamesToPropNames[propName] = propName;
      }
    })
    ...
    if (actualFields) {
      // drop columns
      actualFields.forEach(function (f) {
        const notFound = !~propNames.indexOf(f.Field) && !columnNamesToPropNames[f.Field];
        if (f.Field === idName) return;
        if (notFound || !m.properties[columnNamesToPropNames[f.Field]]) {
  ...
  function datatype(p) {
    ...
    switch (p.type.name) {
      ...
      case `Number`:
        dt = `[${typeLabel || 'int'}]`;
        if (typeof p.precision !== `undefined` || typeof p.scale !== `undefined`) {
